...
export async function getLoginUrl(): Promise<{
  ...
  return {
    ...
    instructions:
      "Please log in to Uber Eats in your browser. After logging in, run the 'ubereats_status' tool to verify authentication and save your session.",
  };
}
 ...
 */
export async function setAddress(address: string): Promise<{
  success: boolean;
  ...
  try {
    ...
    const suggestion = p.locator(
      '[data-testid="address-autocomplete-result"], ' +
      '[role="option"], ' +
    ...
    for (let i = 0; i < Math.min(cardCount, 20); i++) {
      ...
      try {
        const name =
          (await card
            .locator(
              'h3, [data-testid="store-name"], span[class*="StoreName"]'
            )
            .first()
            .textContent()
            .catch(() => "")) || "";
        ...
        const cuisineText =
          (await card
            .locator(
              '[data-testid="store-cuisine"], span[class*="cuisine"]'
            )
            .first()
            .textContent()
            .catch(() => "")) || "";
        ...
        const ratingText =
          (await card
            .locator(
              '[data-testid="store-rating"], span[aria-label*="stars"]'
            )
            .first()
            .textContent()
            .catch(() => "0")) || "0";
        ...
        const feeText =
          (await card
            .locator(
              '[data-testid="store-delivery-fee"], span:has-text("$")'
            )
            .first()
            .textContent()
            .catch(() => "")) || "";
...
export async function getRestaurant(restaurantId: string): Promise<{
  ...
  try {
    ...
    const ratingText =
      (await p
        .locator(
          '[data-testid="store-rating"], span[aria-label*="stars"], span[class*="rating"]'
        )
        .first()
        .textContent()
        .catch(() => "0")) || "0";
    ...
    const deliveryTimeText =
      (await p
        .locator(
          '[data-testid="store-eta"], span:has-text("min delivery")'
        )
        .first()
        .textContent()
        .catch(() => "")) || "";
    ...
    await p
      .locator(
        '[data-testid="menu-item"], [data-testid="store-menu-item-tile"]'
      )
      .first()
    ...
    if (sectionCount > 0) {
      for (let s = 0; s < sectionCount; s++) {
        ...
        const menuItems = section.locator(
          '[data-testid="menu-item"], [data-testid="store-menu-item-tile"]'
        );
        ...
        for (let i = 0; i < Math.min(itemCount, 30); i++) {
          ...
          try {
            const itemName =
              (await item
                .locator(
                  "h3, [data-testid='menu-item-title'], span[class*='ItemName']"
                )
                .first()
                .textContent()
                .catch(() => "")) || "";
            ...
            const description =
              (await item
                .locator(
                  "[data-testid='menu-item-description'], p, span[class*='description']"
                )
                .first()
                .textContent()
                .catch(() => "")) || "";
            ...
            const priceText =
              (await item
                .locator(
                  "[data-testid='menu-item-price'], span:has-text('$')"
                )
                .first()
                .textContent()
                .catch(() => "$0")) || "$0";
...
export async function addToCart(
  ...
  try {
    ...
    if (specialInstructions) {
      const instructionsInput = p.locator(
 ...
 */
export async function viewCart(): Promise<{
  success: boolean;
  items?: CartItem[];
  ...
  try {
    ...
    const emptyCart = await p
      .locator(
        'span:has-text("Your cart is empty"), ' +
 ...
 */
export async function checkout(confirm: boolean = false): Promise<{
  success: boolean;
  orderId?: string;
  summary?: {
    items: CartItem[];
  ...
  try {
    ...
    const items: CartItem[] = [];
    const orderItems = p.locator(
      '[data-testid="checkout-item"], [data-testid="cart-item"]'
    );
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      const name =
        (await item.locator("span, h3").first().textContent().catch(() => "")) ||
        "";
      const quantityText =
        (await item
    ...
    const addressText =
      (await p
        .locator(
          '[data-testid="delivery-address"], span:has-text("Deliver to")'
        )
        .first()
        .textContent()
        .catch(() => "")) || "";
...
export async function trackOrder(orderId?: string): Promise<{
  ...
  try {
    ...
    const etaText =
      (await p
        .locator(
          '[data-testid="delivery-eta"], span:has-text("min"), span:has-text("arriving")'
        )
        .first()
        .textContent()
        .catch(() => "")) || "";
    ...
    for (let i = 0; i < itemCount; i++) {
      const item = orderItems.nth(i);
      const name =
        (await item.locator("span").first().textContent().catch(() => "")) || "";
      const priceText =
        (await item
          .locator('span:has-text("$")')
          .first()
